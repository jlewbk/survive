...
function resolveRound(state) {
  ...
  const tigerAlive = alive.filter((e) => e.card.type === 'tiger');
  ...
  const hunterAlive = alive.filter((e) => e.card.type === 'hunter');
  ...
  if (hunterAlive.length >= 2 && !hasTiger) {
    ...
    alive = alive.filter((e) => e.card.hp > 0);
  }

  // ---- Step 4: 狼灭绝检测 (Wolf Extinction) ----
